...
class ArmKinematicsBridgeRPC(Node):
    ...
    def calculate_ik(self, pose: Pose, gripper_width: float):
        ...
        # Based on physical calibration: Base: 2059 | S_Driv: 2703 | S_Drvn: 1419 | Elbow: 2435 | Wrist: 2038
        SAFE_Z_MIN = 0.03  # Adjusted Z minimum based on visual confirmation of floor
        ...
        # Clamp Base to ±90 degrees from center
        joints[0] = max(1023, min(3071, joints[0]))

        # Shoulder driving angle
        joints[1] = int(2047 + ((distance - 0.2) * 1000))
        # Clamp Shoulder so it cannot move backwards past straight up (vertical = 2047)
        joints[1] = max(2047, min(4095, joints[1]))
        ...
        # --- Gripper Calibration Mapping ---
        # Physical limits: Fully Open = 853, Fully Closed = 2061
        # Width range: 0.08m (open) to 0.0m (closed)
        w_clamped = max(0.0, min(0.08, gripper_width))
        
        if w_clamped <= 0.005:  # Effectively closed
            joints[5] = 2061
        elif w_clamped >= 0.075: # Effectively open
            joints[5] = 853
        else:
            # Linear map: 0.08m -> 853 | 0.0m -> 2061
            # slope = (853 - 2061) / (0.08 - 0.0) = -1208 / 0.08 = -15100
            joints[5] = int(2061 - (w_clamped * 15100))
