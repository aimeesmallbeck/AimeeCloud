#!/usr/bin/env python3
"""Obstacle avoidance test with coarse grid. Robot WILL move."""
...
node = AimeeNavNode(parameter_overrides=[
    ...
    Parameter('enable_planning', Parameter.Type.BOOL, True),
    Parameter('angular_scale', Parameter.Type.DOUBLE, 4.0),
    Parameter('nav_rate_hz', Parameter.Type.DOUBLE, 4.0),
    Parameter('grid_size_m', Parameter.Type.DOUBLE, 3.0),
    Parameter('grid_resolution_m', Parameter.Type.DOUBLE, 0.15),
...
print("=" * 60)
print("OBSTACLE AVOIDANCE TEST — COARSE GRID")
print("Grid: 3.0m x 3.0m @ 0.15m | Nav rate: 4 Hz")
...
for i in range(15):
    ...
    # Show last commanded velocity
    lin = node._rover._last_linear
    ang = node._rover._last_angular
    print(f"  [{i+1:2d}s] front={front_dist:.2f}m fl={fl_dist:.2f}m fr={fr_dist:.2f}m blocked={front_blocked} cmd=({lin:.2f},{ang:.2f})")
